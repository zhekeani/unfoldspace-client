import {
  ...
  EditorDropdownCategoryTitle,
} from "@/components/editor/components/ui/EditorDropdown";
    ...
    <Dropdown.Root>
      <Dropdown.Trigger asChild>
        ...
        >
          ...
          />
          <LucideIcon name="ChevronDown" className="w-2 h-2" />
        </EditorToolbar.Button>
      </Dropdown.Trigger>
          ...
          {options.map((option) => {
            if (isOption(option)) {
              return (
                ...
                >
                  <LucideIcon name={option.icon} className="w-4 h-4 mr-1" />
                  {option.label}
                </EditorDropdownButton>
              );
              ...
              return (
                <div className="mt-2 first:mt-0" key={option.id}>
                  <EditorDropdownCategoryTitle key={option.id}>
                    {option.label}
                  </EditorDropdownCategoryTitle>
                </div>
              );
